...
static std::string GetCommandName(const TreeMap& _gui_map, std::string_view name) {
    ...
    for (std::string_view i : _gui_map) {
        assert((i.find_first_of(" _") == std::string_view::npos));
        ret += '_' + ToLower(i);
    }
    ret += "_";
...
BaseVar::BaseVar(Type _type, TreeMap _gui_map, std::string_view _gui_name)
    : type(_type)
    , gui_map(_gui_map)
    , command_name(GetCommandName(_gui_map, _gui_name))
    , gui_name(std::move(_gui_name)) {
...
Var<bool>::Var(TreeMap _gui_map, std::string_view _gui_name, bool _defaults)
    : BaseVar(BaseVar::Type::kBool, std::move(_gui_map), _gui_name)
    , defaults(_defaults)
    , value(_defaults) { }
...
Var<int>::Var(TreeMap _gui_map, std::string_view _gui_name, int _defaults, int _min, int _max)
    : BaseVar(BaseVar::Type::kInt, std::move(_gui_map), _gui_name)
    , defaults(_defaults)
    , value(_defaults)
    , min(_min)
    , max(_max) { }
...
Var<float>::Var(TreeMap _gui_map, std::string_view _gui_name, float _defaults, float _min, float _max)
    : BaseVar(BaseVar::Type::kFloat, std::move(_gui_map), _gui_name)
    , defaults(_defaults)
    , value(_defaults)
    , min(_min)
    , max(_max) { }
...
Var<Enum>::Var(TreeMap _gui_map, std::string_view _gui_name, int _defaults, Enum _enum)
    : Var<int>(std::move(_gui_map), _gui_name, _defaults, 0, _enum.size() - 1)
    , internal_enum(std::move(_enum)) { assert(!this->internal_enum.empty()); }
...
Var<CatKey>::Var(TreeMap _gui_map, std::string_view _gui_name, CatKey _defaults)
    : BaseVar(BaseVar::Type::kKey, std::move(_gui_map), _gui_name)
    , defaults(_defaults)
    , value(_defaults) { }
