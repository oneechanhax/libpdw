...
Canvas::Canvas() : CBaseWindow("root")  {}
		...
		if (m_bKeysInit) {
			if (changed) {
				...
				if (i == CatKey::CATKEY_MOUSE_1) {
					...
					if (this->IsVisible()) {
						if (down) this->OnMousePress();
						else this->OnMouseRelease();
					}
				} else {
				...
				if (down) {
					...
					if (pressed_time > std::chrono::seconds(1)) {
						...
						if (pressed_time > std::chrono::seconds(4)) {
							...
							if (time_since_keysend > std::chrono::milliseconds(400));
								shouldrepeat = true;
					}
...
static glez::rgba RainbowCurrent() {
	auto FromHSL = [](float h, float s, float v) {
	    ...
	    if (s <= 0.0f) return ToRGBA8(v, v, v);
	    if (h >= 360.0f) h = 0.0f;
	    h /= 60.0f;
	    long i = long(h);
	    float ff = h - i;
