...
void List::Fill(ui::BaseVar* var, int depth) {
	...
	auto find = std::find_if(this->m_children.begin(), this->m_children.end(), [&](auto i){
			return i->GetName() == "ncc_item_sublist" && dynamic_cast<ItemSublist*>(i)->title == var->gui_map[namedepth]; });
...
ui::BaseVar* FindCatVar(const std::string name) {
	for (auto var : ui::BaseVar::GetList()) {
		if (var->command_name == name) return var;
	}
	printf("can't find %s\n", name.c_str());
