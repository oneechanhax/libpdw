...
ItemVariable::ItemVariable(ui::BaseVar& variable) : Item("ncc_item_variable_" + variable.command_name), catvar(variable) {
...
bool ItemVariable::ConsumesKey(CatKey key) {
	...
	if (key == CatKey::CATKEY_M_WHEEL_DOWN || key == CatKey::CATKEY_M_WHEEL_UP || key == CatKey::CATKEY_MOUSE_1) return true;
	return false;
}
...
void ItemVariable::OnKeyPress(CatKey key, bool repeat) {
	...
	if (change < 1.0f && catvar.type == ui::BaseVar::Type::kInt) change = 1.0f;
